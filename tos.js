// tos.js
// BetnixTech Terms of Service Script

const betnixTOS = `
<h1>Betnix Terms of Service</h1>
...
// Function to display TOS in a container
function showTOS(containerId) {
    const container = document.getElementById(containerId);
    if (!container) {
        console.error(`Container with id "${containerId}" not found.`);
        return;
    }
    container.innerHTML = betnixTOS;
}

// Optional: Automatically display in a div with id="tos" when loaded
document.addEventListener("DOMContentLoaded", () => {
    const tosContainer = document.getElementById("tos");
    if (tosContainer) {
        tosContainer.innerHTML = betnixTOS;
    }
});
